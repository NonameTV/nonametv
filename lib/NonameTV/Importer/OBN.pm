...
#print ">$text<\n";
  ...
  if( $text =~ /^(ponedjeljak|utorak|srijeda|ČETVRTAK|petak|subota|nedjelja),\s*\d+\.\s*(siječanj|veljače|mart|april|svibanj|lipanj|srpanj|kolovoz|rujan|listopad|studeni|prosinac)\s*\d+\.$/i ){
    return 1;
  } elsif( $text =~ /^(ponedjeljak|utorak|srijeda|ČETVRTAK|petak|subota|nedjelja),\s*\d+\.\s*(siječanj|veljače|mart|april|svibanj|lipanj|srpanj|kolovoz|rujan|listopad|studeni|prosinac)$/i ){ # format 'SUBOTA, 21. studeni'
    return 1;
  }
