...
use constant {
  ...
  FT_GRIDXLSD => 3,  # xls file with grid - with dates (NG WILD)
};
...
sub ImportContentFile {
  ...
  } elsif( $ft eq FT_GRIDXLSD ){
    $self->ImportGridXLSD( $file, $chd );
  }
...
#print "$iR $iC " . $oWkC->Value . "\n";
        ...
        return FT_FLATXLS if( $oWkC->Value =~ /^CROATIAN$/ );
      }
    }
  }
  ...
  for(my $iW = 0 ; $iW <= $oBook->{SheetCount} ; $iW++) {
    ...
    for(my $iR = 0 ; $iR <= 5 ; $iR++) {
      for(my $iC = 0 ; $iC <= 8 ; $iC++) {
        ...
        next if( ! $oWkC->Value );
#print "$iR $iC " . $oWkC->Value . "\n";
        return FT_GRIDXLSD if( $oWkC->Value =~ /SUNDAY/ );
      }
    }
  }
...
sub ImportFlatXLS
{
  ...
  progress( "NGC Flat XLS: $channel_xmltvid: Processing $file" );

  my( $oBook, $oWkS, $oWkC );
  $oBook = Spreadsheet::ParseExcel::Workbook->Parse( $file );

  if( not defined( $oBook ) ) {
    error( "NGC Flat XLS: $file: Failed to parse xls" );
    return;
  }
  ...
  for(my $iSheet=0; $iSheet < $oBook->{SheetCount} ; $iSheet++) {
    ...
    progress("NGC Flat XLS: $channel_xmltvid: processing worksheet named '$oWkS->{Name}'");
      ...
      if( not %columns ){
        ...
        for(my $iC = $oWkS->{MinCol} ; defined $oWkS->{MaxCol} && $iC <= $oWkS->{MaxCol} ; $iC++) {
          if( $oWkS->{Cells}[$iR][$iC] ){
            ...
            $columns{DATE} = $iC if( $oWkS->{Cells}[$iR][$iC]->Value =~ /CET/ );
            ...
            if( $oWkS->{Cells}[$iR][$iC]->Value =~ /CROATIAN/ ){
              $columns{PROGRAM} = $iC;
              $found = 1;
            }
...
#}
      ...
      if( isDate( $oWkC->Value ) ){
	    ...
	    $dsh->EndBatch( 1 );
          ...
          progress("NGC Flat XLS: $channel_xmltvid: Date is: $date");
        }
      ...
      progress( "NGC Flat XLS: $channel_xmltvid: $time - $title" );
...
sub ImportGridXLS
{
  ...
  # Only process .xls files.
  return if( $file !~ /\.xls$/i );
  progress( "NGC GridXLS: $xmltvid: Processing $file" );
  ...
  if( ! $month or ! $year ){
    error( "NGC GridXLS: $xmltvid: Error extracting month and year from filename" );
    return;
  }
  ...
  for(my $iSheet=0; $iSheet < $oBook->{SheetCount} ; $iSheet++) {
    ...
    if( $oWkS->{Name} !~ /^W/i ){
      progress( "NGC GridXLS: $xmltvid: Skipping worksheet: $oWkS->{Name}" );
      next;
    }
    progress( "NGC GridXLS: $xmltvid: Processing worksheet: $oWkS->{Name}" );

#    ( $firstdate, $lastdate ) = ParsePeriod( $oWkS->{Name} );
#    progress( "NGC GridXLS: $xmltvid: Importing data for period from " . $firstdate->ymd("-") . " to " . $lastdate->ymd("-") );
#    my $period = $lastdate - $firstdate;
#    my $spreadweeks = int( $period->delta_days / 7 ) + 1;
#    if( $period->delta_days > 6 ){
#      progress( "NGC GridXLS: $xmltvid: Schedules scheme will spread accross $spreadweeks weeks" );
#    }
...
print "DAY $day DAYNO $dayno\n";
          ...
          if( $dayno eq 0 ){
            if( $day eq 1 ){
              ...
              progress( "NGC GridXLS: $xmltvid: Skipping day from the previous month: $day" );
            }
          } else {
            $dayno = $day;
            $firstdate = sprintf( "%04d-%02d-%02d", $year, $month, $day ) if not $firstdate;
...
sub ImportGridXLSD
{
  ...
  # Only process .xls files.
  return if( $file !~ /\.xls$/i );
  progress( "NGC GridXLSD: $chd->{xmltvid}: Processing $file" );
  ...
  for(my $iSheet=0; $iSheet < $oBook->{SheetCount} ; $iSheet++) {
    ...
    if( $oWkS->{Name} !~ /^W/i ){
      progress( "NGC GridXLSD: $chd->{xmltvid}: Skipping worksheet: $oWkS->{Name}" );
      next;
    }
    progress( "NGC GridXLSD: $chd->{xmltvid}: Processing worksheet: $oWkS->{Name}" );
      ...
      for(my $iR = $firstrow ; defined $oWkS->{MaxRow} && $iR <= $oWkS->{MaxRow} ; $iR++) {
        ...
        if( isDate( $text ) ){
          ...
          if( $date ne $currdate ) {
            ...
            $dsh->StartDate( $date, "08:00" );
            $currdate = $date;
            progress( "NGC GridXLSD: $chd->{xmltvid}: Date is $date" );
          }
        ...
        progress( "NGC GridXLSD: $chd->{xmltvid}: $time - $title" );
...
print "FlushData Last  DATE $ldt\n";
  ...
  foreach my $dayshows ( @shows ) {
    ...
    progress( "NGC GridXLS: $xmltvid: Date is " . $date->ymd() );
    ...
    foreach my $s ( @{$dayshows} ) {
      ...
      progress( "NGC GridXLS: $xmltvid: $s->{start_time} - $s->{title}" );
...
#print "isDate >$text<\n";
  ...
  if( $text =~ /^\d{4}-\d{2}-\d{2}$/ ){ # the format is '2010-04-25'
    return 1;
  } elsif( $text =~ /^\d+-\d+-\d+$/ ){ # the format is '01-10-08'
    return 1;
...
print "ParseDate >$text<\n";
  ...
  } elsif( $text =~ /^\d{2}-\d{2}-\d{2}$/ ){ # the format is '26-01-11'
    ( $day, $month, $year ) = ( $text =~ /^(\d+)-(\d+)-(\d+)$/ );
  } elsif( $text =~ /^\d+-\d+-\d+$/ ){ # the format is '1-26-11'
    ( $month, $day, $year ) = ( $text =~ /^(\d+)-(\d+)-(\d+)$/ );
  } elsif( $text =~ /^\d{2}-\d{2}-\d{2}$/ ){ # the format is '01-10-08'
    ( $day, $month, $year ) = ( $text =~ /^(\d+)-(\d+)-(\d+)$/ );
  } elsif( $text =~ /^(sunday|monday|tuesday|wednesday|thursday|friday|saturday)\s+(\d+)$/i ){
    ( $dayname, $day ) = ( $text =~ /^(\S+)\s+(\d+)$/ );
    return $day;
  } elsif( $text =~ /^(ponedjeljak|utorak|srijeda|Četvrtak|petak|subota|nedjelja)\s+(\d+)$/i ){
    ( $dayname, $day ) = ( $text =~ /^(\S+)\s+(\d+)$/ );
    return $day;
