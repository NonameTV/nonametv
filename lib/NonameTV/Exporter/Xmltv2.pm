package NonameTV::Exporter::Xmltv2;
...
sub new {
    ...
    $self->{OptionSpec} = [ qw/export-channels remove-old force-export append-credits 
			    epgserver=s exportedlist=s channel-group=s
			    verbose+ quiet+ help/ ];
...
EOH
  ...
  if( $exportedlist ){
    ...
    progress("Xmltv2: The list of exported files will be available in '$exportedlist'");
  }
  ...
  my $equery = "SELECT * from epgservers WHERE `active`=1 AND `type`='Xmltv'";
  ...
  while( my $edata = $esth->fetchrow_hashref() )
  {
    ...
    progress("Xmltv2: Exporting schedules for services on epg server '$edata->{name}'");
    ...
    while( my $ndata = $nsth->fetchrow_hashref() )
    {
      ...
      while( my $sdata = $ssth->fetchrow_hashref() )
      {
        foreach my $channel (keys %{$todo}) {
          ...
          if ( $sdata->{dbchid} eq $channel ){
            progress("Xmltv2: Exporting service $ndata->{id}/$sdata->{serviceid} - $sdata->{servicename} (channel $channel)");
            $todofiltered->{$channel} = $todo->{$channel};
          }
        }
      }
      $ssth->finish();
    ...
    $self->ExportData( $todofiltered );
    $self->WriteState( $update_started );
  }
  $esth->finish();

  EndLogSection( "Xmltv2" );
}
...
sub ReadState {
  ...
  my $last_update = $ds->sa->Lookup( 'state', { name => "xmltv2_last_update" },
                                 'value' );

  if( not defined( $last_update ) )
  {
    $ds->sa->Add( 'state', { name => "xmltv2_last_update", value => 0 } );
    $last_update = 0;
  }
...
sub WriteState {
  ...
  $ds->sa->Update( 'state', { name => "xmltv2_last_update" }, 
...
sub create_dt
{
  ...
  die( "Xmltv2: Unknown time format $str" )
    unless defined $day;
...
sub ExportFile {
  ...
  my $section = "Xmltv2 $chd->{xmltvid}_$date";
...
sub CreateWriter
{
  ...
  my $path = $self->{Root} . "/" . $self->{epgserver} . "/";
  ...
  open( my $fh, '>:encoding(' . $self->{Encoding} . ')', "$path$filename.new")
    or die( "Xmltv2: cannot write to $path$filename.new" );
...
sub ExportChannelList
{
  ...
  open( my $fh, '>', $outfile )
    or die( "Xmltv2: cannot write to $outfile" );
...
sub RemoveOld
{
  ...
  my @files = glob( $self->{Root} . "/" . $self->{epgserver} . "/*" );
  my $removed = 0;
